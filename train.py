...
from data_gen import load_test_data_for_rnn, load_test_data_for_rnn1, load_train_data_for_rnn, load_train_data_for_rnn1, \
    load_test_data_for_cnn, load_train_data_for_cnn, erath_data_transform, sea_mask_rnn, sea_mask_cnn, sea_mask_rnn1, \
    load_train_data_for_CS, load_test_data_for_CS, load_train_data_for_rnn_nomal, load_test_data_for_rnn_nomal
...
def train(x,
    ...
    if cfg['modelname'] in ['MHLSTMModel', 'MSLSTMModel', "SoftMTLv1", "MMOE", "LSTM", "REDFLSTM", "FAMLSTM", "EDLSTM"]:
        ...
        if valid_split:
            x_valid_lstm, y_valid_lstm, static_valid_lstm, mask_index_val = sea_mask_rnn1(cfg, x_valid, y_valid,
                                                                                          static_valid, mask)
    ...
    for num_ in range(1):
        ...
        if cfg['modelname'] in ['FAMLSTM']:
            ...
            model_And = FAMLSTM(cfg, REDFLSTM_cfg).to(device)
        if cfg['modelname'] in ['EDLSTM']:
            ...
            gauss_modelQ = TransformerVAE(feature_dim=128, hidden_dim=cfg["hidden_size"], num_heads=2, num_layers=2,
                                          dropout_rate=cfg["dropout_rate"]).to(device)
            gauss_modelP = TransformerVAE(feature_dim=128, hidden_dim=cfg["hidden_size"], num_heads=2, num_layers=2,
                                          dropout_rate=cfg["dropout_rate"]).to(device)
        ...
        if cfg["modelname"] in \
            ...
            log_sigma1 = torch.nn.Parameter(torch.zeros(1, device=device, requires_grad=True))
            log_sigma2 = torch.nn.Parameter(torch.zeros(1, device=device, requires_grad=True))
            log_sigma3 = torch.nn.Parameter(torch.zeros(1, device=device, requires_grad=True))
            log_sigma4 = torch.nn.Parameter(torch.zeros(1, device=device, requires_grad=True))

            # 主要模型优化器（包含不确定性参数）
            optim = torch.optim.Adam([
                {'params': model.parameters()},
                {'params': [log_sigma1, log_sigma2, log_sigma3, log_sigma4]}
            ], lr=cfg['learning_rate'])

            optim_And = torch.optim.Adam(model_And.parameters(), lr=cfg['learning_rate'])
        if cfg["modelname"] in \
                ["MTANmodel"]:
            sigma = []
        ...
        with trange(1, cfg['epochs'] + 1) as pbar:
            for epoch in pbar:
                ...
                for iter in range(0, cfg["niter"]):
                    ...
                    if cfg["modelname"] in \
                        ...
                        for pre_num in range(cfg['num_repeat']):
                            ...
                            x_batch[pre_num] = x_batch[pre_num].reshape(x_batch[pre_num].shape[0],
                                                                        x_batch[pre_num].shape[1] *
                                                                        x_batch[pre_num].shape[2],
                                                                        x_batch[pre_num].shape[3],
                                                                        x_batch[pre_num].shape[4])
                        ...
                        x_cnn, y_cnn, aux_cnn, x_lstm, y_lstm, aux_lstm, x_ture = \
                            load_train_data_for_cnn(cfg, x, y, static, scaler_y, x_lst, y_lst, static_lst, lat_index,
                                                    lon_index, mask_index)

                        predictions = []
                        # CNN的输入是batch 时间 特征数 7*7的一个空间特征图  这个特征图表达的是7*7这个区域的某一个特征
                        # lstm的是 batch 时间 特征数  这里是某一个点的某天的所有特征
                        for pre_num in range(cfg['num_repeat']):
                            # 通过可学习的高斯分布生成噪声
                            if Gauss:
                    ...
                    #     optimH.step()
                    ...
                    elif cfg['modelname'] in ['MTLCNN', 'MTLConvLSTMModel', "CrossStitchNet", "MTANmodel", "REDFLSTM",
                                              "FAMLSTM", "EDLSTM"]:
                        ...
                        # 从输入数据中提取降雨和径流数据
                        input_data = xx[0]  # [batch_size, seq_len, features]
                        precipitation = input_data[:, :, 3]  # 提取降雨数据
                        runoff = input_data[:, :, 9]  # 提取径流数据
                        # 取时间序列的最后一个时间步作为当前值
                        P = precipitation[:, -1]  # [batch_size]
                        R = runoff[:, -1]  # [batch_size]
                        Ax = []
                        # And模型预测残差

                        Ax.append(input_data)
                        and_pred = model_And(Ax,input_data,cfg)  # And模型前向传播
                        And_pred = and_pred[0].squeeze()
                        ...
                        for i in range(4):
                            ...
                            losses.append(loss_time)
                            supervised_loss += loss_time
                        #     不确定性加权
                        weighted_loss_task1 = torch.exp(log_sigma1) * losses[0] +log_sigma1
                        weighted_loss_task2 = torch.exp(log_sigma2) * losses[1] +log_sigma2
                        weighted_loss_task3 = torch.exp(log_sigma3) * losses[2] +log_sigma3
                        weighted_loss_task4 = torch.exp(log_sigma4) * losses[3] +log_sigma4
                        supervised_loss = weighted_loss_task1 + weighted_loss_task2 + weighted_loss_task3 + weighted_loss_task4

                        # 物理约束损失计算
                        # 物理约束：降雨 - 蒸散 - 径流 - 3层土壤湿度 + And = 0
                        physics_loss = 0.0
                        physics_loss_type = cfg.get('physics_loss_type', 'l1')  # 可选: 'mse', 'l1', 'rmse'

                        # 从神经网络输出中提取蒸散和土壤湿度
                        pred_output = pred
                        SM1 = pred_output[0].squeeze()  # 第1层土壤湿度
                        SM2 = pred_output[1].squeeze()  # 第2层土壤湿度
                        SM3 = pred_output[2].squeeze()  # 第3层土壤湿度
                        ET = pred_output[3].squeeze()  # 蒸散

                        SM_total = SM1 + SM2 + SM3  # 3层土壤湿度总和

                        # 计算物理约束：降雨 - 蒸散 - 径流 - 3层土壤湿度 + And = 0
                        physics_constraint = P - ET - R - SM_total + And_pred

                        # 根据配置选择损失函数类型
                        if physics_loss_type == 'mse':
                            physics_loss += torch.mean(physics_constraint ** 2)
                        elif physics_loss_type == 'l1':
                            physics_loss += torch.mean(torch.abs(physics_constraint))
                        elif physics_loss_type == 'rmse':
                            physics_loss += torch.sqrt(torch.mean(physics_constraint ** 2))
                        else:
                            # 默认使用MSE
                            physics_loss += torch.mean(physics_constraint ** 2)

                        # 主模型总损失
                        sup_weight = cfg.get('supervised_weight', 1.0)
                        physics_weight = cfg.get('physics_loss_weight', 0.01)  # 物理约束损失权重
                        main_total_loss = sup_weight * supervised_loss + physics_weight * physics_loss

                        optim.zero_grad()
                        main_total_loss.backward(retain_graph=True)  # 多任务模型梯度
                        torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
                        optim.step()

                        # And模型损失：纯物理约束（无权重）
                        physics_constraint_and =  P - ET.detach() - R - SM_total.detach() + And_pred
                        and_model_loss = torch.mean(physics_constraint_and ** 2)

                        optim_And.zero_grad()
                        and_model_loss.backward()  # And模型梯度
                        torch.nn.utils.clip_grad_norm_(model_And.parameters(), max_norm=1.0)
                        optim_And.step()


                        total_loss = main_total_loss

                        MSELoss += total_loss.item()
                ...
                if cfg["modelname"] in ['MSLSTMModel', 'MHLSTMModel', 'SoftMTLv1', 'MMOE']:
                    ...
                    #                                                                t_end - t_begin)
                    print(loss_str)
                elif cfg["modelname"] in ['MTLCNN', 'MTLConvLSTMModel', 'MTANmodel', 'CrossStitchNet', "REDFLSTM",
                                          "FAMLSTM", "EDLSTM"]:
                    # 打印每个任务的损失值和训练时间
                    t_end = time.time()
                    ...
                    if epoch % 10 == 0:
                        ...
                        if cfg["modelname"] in ['MSLSTMModel']:
                            ...
                            for i in range(0, n):
                                ...
                                x_valid_batch, y_valid_batch, aux_valid_batch, _, _ = \
                                    load_test_data_for_rnn1(cfg, x_valid, y_valid, static_valid, scaler_y,
                                                            cfg["stride"], i, n)
                                yy = []
                                xx = []
                                pred_valid = []
                                ...
                                for ini in range(cfg['num_repeat']):
                                    loss_ = NaNMSELoss.fit(cfg, pred_valid[ini].squeeze().float(),
                                                           yy[ini].squeeze().float(), lossmse)
                        ...
                        if cfg["modelname"] in ['FAMLSTM', "EDLSTM"]:  # FAMLSTM,REDFLSTM
                            ...
                            for i in range(0, n):
                                # mask
                                x_valid_batch, y_valid_batch, aux_valid_batch, _, _ = \
                                    load_test_data_for_rnn_nomal(cfg, x_valid, y_valid, static_valid, scaler_y,
                                                                 cfg["stride"], i, n, mask_index_val)
                                yy = []
                                xx = []
                                pred_valid = []
                                for ii in range(len(y_valid_batch)):
                            ...
                            gt_list = [i for i in
                                       range(0, x_valid.shape[0] - cfg['seq_len'] - cfg['forcast_time'], cfg["stride"])]
                            ...
                            for i in gt_list:
                                x_valid_batch, y_valid_batch, aux_valid_batch, _, _ = \
                                    load_test_data_for_cnn(cfg, x_valid, y_valid, static_valid, scaler_y, gt_list,
                                                           ...
                                                           cfg["stride"])  # same as Convolutional Neural Network
                                for num in range(cfg['num_repeat']):
                                    ...
                                    x_valid_batch[num] = x_valid_batch[num].reshape(x_valid_batch[num].shape[0],
                                                                                    x_valid_batch[num].shape[1] *
                                                                                    x_valid_batch[num].shape[2],
                                                                                    x_valid_batch[num].shape[3],
                                                                                    x_valid_batch[num].shape[4])
                            ...
                            for i in gt_list:
                                x_valid_batch, y_valid_batch, aux_valid_batch, _, _ = \
                                    load_test_data_for_CS(cfg, x_valid, y_valid, static_valid, scaler_y, gt_list,
                                                          lat_index, lon_index, i,
                                                          cfg["stride"])  # same as Convolutional Neural Network
                                for num in range(cfg['num_repeat']):
                                    x_valid_batch[num][np.isnan(x_valid_batch[num])] = 0
                            ...
                            gt_list = [i for i in range(0,
                                                        x_valid.shape[0] - cfg['seq_len'] - cfg['forcast_time'] - cfg[
                                                            "multi_step"], cfg["stride"])]
                            n = (x_valid[0].shape[0] - cfg["seq_len"]) // cfg["stride"]
                            valid_batch_size = cfg["batch_size"] * 10
                            for i in gt_list:
                                ...
                                x_cnn, y_cnn, aux_cnn, _, _, mask_cnn = \
                                    load_test_data_for_cnn(cfg, x_valid, y_valid, static_valid, scaler_y, gt_list,
                                                           lat_index, lon_index, i, cfg["stride"],
                                                           mask_index)  # same as Convolutional Neural Network
                                ...
                                for pre_num in range(cfg['num_repeat']):
                                    ...
                                    zeros_tensor = torch.zeros(x_lstm[pre_num].size(0), x_lstm[pre_num].size(1),
                                                               128).to(device)
                                ...
                                with torch.no_grad():
                                    ...
                                    #         loss_sum = loss_sum + loss_time
                                    for i_val in range(cfg['num_repeat']):
                                        mse_valid_loss = NaNMSELoss.fit(cfg, pred_valid[i_val].squeeze().float(),
                                                                        y_lstm[i_val][:, 0].float(),
                                                                        lossmse)
                            ...
                            for i in gt_list:
                                ...
                                for num in range(cfg['num_repeat']):
                                    ...
                                    aux_valid_batch[num] = aux_valid_batch[num].repeat(1, x_valid_batch[num].shape[1],
                                                                                       1, 1, 1)
                        ...
                        if cfg["modelname"] in ['LSTM']:
                            ...
                            for i in range(0, n):
                                ...
                                if isinstance(y_valid, list):
                                    y_valid = y_valid[0]
                                x_valid_batch, y_valid_batch, aux_valid_batch, _, _ = \
                                    load_test_data_for_rnn(cfg, x_valid, y_valid, static_valid, scaler_y, cfg["stride"],
                                                           i, n)
                        ...
                        if cfg["modelname"] in ['MSLSTMModel', 'SoftMTLv1', 'MMOE', 'MTLCNN', 'MTLConvLSTMModel',
                                                'CrossStitchNet', 'REDFLSTM', "MTANmodel", "FAMLSTM", "EDLSTM"]:
                            t_end = time.time()
