...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        output, (hidden, cell) = self.lstm(x)  # 返回hidden和cell状态
        return output, (hidden, cell)  # 修改这里返回cell状态也
...
class Decoder(nn.Module):
    ...
    def forward(self, Rinput, encoder_hidden):
        # 使用encoder的hidden和cell状态初始化decoder的LSTM
...
class FAMLSTM(nn.Module):
    ...
    def __init__(self, cfg, lstmmodel_cfg):
        ...
        self.lstm1 = nn.LSTM(15, lstmmodel_cfg["hidden_size"], batch_first=True)
    ...
    def forward(self, inputs,aux,cfg):
        ...
        for i in range(len(inputs_)):
           # Combine encoder output with decoder output and the auxiliary input
            ...
            # Pass combined inputs through second LSTM layer
            lstm_output1, _ = self.lstm1(inputs_[i].float())
...
class EDLSTM(nn.Module):
    ...
    def __init__(self, cfg, lstmmodel_cfg):
        ...
        self.gates = nn.ModuleList([Gate(cfg["hidden_size"]) for _ in range(cfg["num_repeat"])]) # 定义两个门控
        for i in range(cfg['num_repeat']):
            # dense的时候 需要将数据变成拉成一条直线 所以是通道数256*长和宽
